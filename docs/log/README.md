...
phase in [TRACKER.md](../specs/TRACKER.md) — which is most day-to-day fixes and
...
Until the 2026-08-30 split these lived at `specs/log/`; dated entries still
refer to files by their old `specs/…` paths, which is deliberate — they are a
...
slug, and `ls docs/log` sorts them chronologically. `grep` the headings when a
slug is not enough.
