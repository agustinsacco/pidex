...
- **No models available** (`get_available_models` empty): guide to authenticate — one-click "open terminal running `pi`" (built-in terminal, [06-terminal.md](../../terminal.md)) for `/login` OAuth flows, or point at env vars / `models.json` for API keys and local endpoints. Never handle or display secrets in pidex UI.
