...
command palette or `⌘\`` (which is _also_ broken, see A2), so on a fresh
...
right — i.e. the model and effort pickers are present _before_ a session
...
_shape_ with what's real: avatar (initials from `app:userInfo`), machine
...
deliberately _not_ the terracotta accent. pidex uses `--px-info`
...
`max-w-3xl` (768px) _including_ `px-6`, so the effective measure is ~720px
