...
- **IPC design**: request/response methods (`invoke`) for commands, push channels (`send`) for streams. Namespace per domain: `pi:*` (session lifecycle + RPC passthrough + event stream), `pty:*`, `fs:*`, `git:*`, `app:*` (prefs, dialogs, theme). Every message type declared in a shared `types/ipc.ts`.
...
│  ├─ main.ts
│  ├─ preload.ts
│  ├─ pi/                      # PiRpcClient, session registry, jsonl framing
│  ├─ pty/
│  ├─ fs/                      # fs service, watchers, git status
│  └─ store.ts                 # app prefs (electron-store)
├─ src/                        # renderer
│  ├─ app/                     # shell, panes, routing
...
- App prefs (theme, layout, workspaces, font size) in electron-store — never written into pi's config files. pi config editing is explicit and user-initiated ([09-settings.md](09-settings.md)).
