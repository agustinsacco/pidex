...
| Install              | GitHub Releases + `curl … install.sh \| sh` (see [10-packaging.md](../build/10-packaging.md))    |
...
- **IPC design**: request/response methods (`invoke`) for commands, push channels (`send`) for streams. Namespace per domain, 13 prefixes today: `pi:*` (session lifecycle + RPC passthrough + event stream), `sessions:*`, `fleet:*`, `orchestrator:*`, `pty:*`, `fs:*`, `git:*`, `gh:*`, `mcp:*`, `clipboard:*`, `packages:*`, `updates:*`, `app:*` (prefs, dialogs, theme). Every message type is declared in [`shared/ipc.ts`](../../shared/ipc.ts).
