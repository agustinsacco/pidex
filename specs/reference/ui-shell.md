...
4. **Artifacts pane** — gallery + viewer ([07-artifacts.md](../build/07-artifacts.md)); opens automatically when a session produces its first artifact.
...
- **Empty states**: no workspace → warm onboarding card with folder picker; no sessions → "Describe a task…" hero; pi missing/outdated → setup screen with install command ([08-sessions.md](../build/08-sessions.md)).
